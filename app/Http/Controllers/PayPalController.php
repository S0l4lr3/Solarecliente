...
class PayPalController extends Controller
{
    ...
    public function payment(Request $request)
    {
        ...
        // --- PRIORIDAD FINANCIERA: EL TOTAL YA CALCULADO CON IVA Y ENVÍO ---
        $monto = $request->monto ?? session('monto_pedido');
        $id_pedido = $request->id_pedido ?? session('last_order_id');

        if (!$monto || $monto <= 0) {
            return redirect()->route('carrito')->with('error', 'Error en el cálculo del total.');
        }

        // Guardamos para recuperarlo al volver
        session(['last_order_id' => $id_pedido, 'monto_pedido' => $monto]);
    ...
    public function success(Request $request)
    {
        ...
        try {
            ...
            if ($response->successful() && $response->json()['status'] == 'success') {
                ...
                $totalPagado = $data['total_final'] ?? 0;
                ...
                return view('cliente.pago_exitoso', compact('pedido', 'paypalTransactionId', 'totalPagado'));
            }

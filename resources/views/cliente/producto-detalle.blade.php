        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 5rem; align-items: start;">
            ...
            <div>
                ...
                <div style="background-color: #f9f9f9; overflow: hidden; border-radius: 4px;">
                    <img id="mainProductImage" src="{{ $imagenUrl }}" alt="{{ $mueble['nombre'] }}" 
                         style="width: 100%; height: 600px; object-fit: cover; transition: opacity 0.3s ease;">
                </div>
                {{-- @dd($mueble) --}}
                @if(isset($mueble['imagenes']) && count($mueble['imagenes']) > 1)
                    <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; margin-top: 1rem;">
                        @foreach($mueble['imagenes'] as $img)
                            <img src="{{ $img['full_url'] ?? $img['url'] }}" 
                                 onclick="changeMainImage(this.src)"
                                 style="width: 100%; height: 100px; object-fit: cover; cursor: pointer; border: 1px solid #eee; transition: transform 0.2s;"
            ...
            <div style="padding-top: 2rem;">
                <nav
                    style="margin-bottom: 2rem; font-size: 10px; text-transform: uppercase; letter-spacing: 2px; color: #999;">
                ...
                <div style="border-top: 1px solid #edebe8; padding-top: 2rem; margin-bottom: 3rem;">
                    <h4
                        style="font-size: 10px; font-weight: bold; text-transform: uppercase; letter-spacing: 2px; margin-bottom: 1rem; color: #958174;">
                        Descripción del Mueble</h4>
